...
def _generate_high_similarity_pair(
    ...
    a.flat[list(set_a)] = 1.0
    b.flat[list(set_b)] = 1.0
    return a, b
...
def _compute_metrics(pred: np.ndarray, true: np.ndarray) -> dict:
    """Compute MAE, RMSE, Pearson r, and Spearman ρ."""
...
def _compute_accuracy_results(
    ...
    return {
        "tensorized_kron": _compute_metrics(kron_j, exact_j),
        "tt_decomposition": _compute_metrics(tt_j, exact_j),
        "datasketch_baseline": _compute_metrics(ds_j, exact_j),
...
def _hash_pair(
    ...
    ds = datasketch_jaccard(a, b, num_perms=num_hashes)
...
def benchmark_accuracy_from_tensors(
    ...
    for i, j in pairs:
        ...
        ds_j.append(datasketch_jaccard(a, b, num_perms=num_hashes))
