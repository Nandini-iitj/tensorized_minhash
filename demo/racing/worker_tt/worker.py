"""
worker.py - Tensor Train (TT) MinHash racing worker.

Reads tensor pairs from /shared_input/, computes TT-MinHash signatures,
...
Progress lines (read by Streamlit via docker logs):
    ...
    DONE: algo=tt load_level=7 elapsed=5.12s mae=0.018
"""
...
from tensorized_minhash.core import TTDecomposedMinHash, TTMinHashConfig
...
def main():
    ...
    logger.info(
        "Hasher built: %d KB compression=%dx", 
        mem["tt_bytes"] // 1024, 
    ...
    for i in range(n_pairs):
        ...
        tt_j = hasher.jaccard_from_signatures(sig_a, sig_b)
    ...
    print(
        f"DONE: algo=tt load_level={load_level} elapsed={elapsed:.2f}s mae={mae:.4f}", 
        flush=True
    )

    # Write metrics file
    timestamp = int(time.time())
    out_path = SCORES_DIR / f"tt_{timestamp}_level{load_level}_metrics.json"
    metrics = {
        "algo": "tt",
